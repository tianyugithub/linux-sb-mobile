...
export async function solveProofOfWork(nonce: string, difficulty: number): Promise<string> {
  ...
  for (let value = 0; value < 5_000_000; value += 1) {
    const candidate = String(value);
    const digest = await sha256Hex(`${nonce}:${candidate}`);
    if (digest.startsWith(prefix)) return candidate;
    if (value % 64 === 0) await new Promise((resolve) => setTimeout(resolve, 0));
  }
  throw new Error('CAPTCHA_TIMEOUT');
}

export async function checkProofOfWork(nonce: string, solution: string, difficulty: number): Promise<boolean> {
  const prefix = '0'.repeat(Math.max(1, difficulty));
